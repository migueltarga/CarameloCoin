...
const char * const BITCOIN_CONF_FILENAME = "caramelocoin.conf";
const char * const BITCOIN_PID_FILENAME = "caramelocoind.pid";
...
#else
    const char* pszModule = "caramelocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CarameloCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CarameloCoin";
#else
    // Unix
    return pathRet / ".caramelocoin";
#endif
#endif
}
